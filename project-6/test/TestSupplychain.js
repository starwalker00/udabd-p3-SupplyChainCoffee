...
contract('SupplyChain', function(accounts) {
    ...
    // Initialize roles
    it("Initialize roles", async() => {

        const farmerRole = await FarmerRole.deployed()
        await farmerRole.addFarmer(originFarmerID, {from: ownerID})
        var isFarmer = await farmerRole.isFarmer(originFarmerID)

        const distributorRole = await DistributorRole.deployed()
        await distributorRole.addDistributor(distributorID, {from: ownerID})
        var isDistributor = await distributorRole.isDistributor(distributorID)
        
        const retailerRole = await RetailerRole.deployed()
        await retailerRole.addRetailer(retailerID, {from: ownerID})
        var isRetailer = await retailerRole.isRetailer(retailerID)

        const consumerRole = await ConsumerRole.deployed()
        await consumerRole.addConsumer(consumerID, {from: ownerID})
        var isConsumer = await consumerRole.isConsumer(consumerID)

        // Verify the result set
        assert.equal(isFarmer, 1, 'Error: Invalid Farmer role')
        assert.equal(isDistributor, 1, 'Error: Invalid Distributor role')
        assert.equal(isRetailer, 1, 'Error: Invalid Retailer role')
        assert.equal(isConsumer, 1, 'Error: Invalid Consumer role')
    })  
    ...
    it("Testing smart contract function harvestItem() that allows a farmer to harvest coffee", async() => {
        ...
        // Verify the result set
        console.log("resultBufferOne:" + JSON.stringify(resultBufferOne));
        console.log("resultBufferTwo:" + JSON.stringify(resultBufferTwo));
